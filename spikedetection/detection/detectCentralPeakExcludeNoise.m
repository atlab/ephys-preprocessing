function sdt = detectCentralPeakExcludeNoise(sdt, varargin)
% Detect spikes as local maxima above threshold on central channels.
...
% detect local maxima above threshold (excluding first and last channel)
above = any(bsxfun(@gt, x(:, 2 : end - 1), thresh(2 : end - 1)), 2);
dr = diff(r);
