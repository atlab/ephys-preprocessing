function extract_electrode_lfp(inPath, outPath, varargin)
...
mkdir(getLocalPath(outPath));
fp = H5Tools.createFile( fullfile(getLocalPath(outPath), 'lfp%d'), 'driver', 'family' );

% Limit memory usage
targetSize = 100 * 2.^20;       % 100 MB chunks
blockSize = ceil( targetSize / 10 / 96) ;
...
for p=1:length(pr)
    ...
    if  mod(p,round((length(pr) / 20))) == 0
        disp(['LFP Extraction: ' num2str(round(p * 100 / length(pr))) '%']);
    end
end
...
H5Tools.writeAttribute(rootDest, 'Fs', samplingRate / prod(factors));

try
    gain = H5Tools.readAttribute(rootSource, 'scale');
catch
